...
interface TrustRiskChartProps {
  ...
  awaitingFirstExecution?: boolean;
}

export const TrustRiskChart: React.FC<TrustRiskChartProps> = ({
  trustScore,
  riskScore,
  driftDetected,
  awaitingFirstExecution = false,
}) => {
  return (
      ...
      <div className="glass-panel" style={{ padding: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
          ...
          <span
            style={{
              ...
              padding: '2px 8px',
              borderRadius: '4px',
              backgroundColor: awaitingFirstExecution
                ? 'rgba(255, 255, 255, 0.06)'
                : driftDetected
                ? 'rgba(245, 158, 11, 0.15)'
                : 'rgba(16, 185, 129, 0.15)',
              color: awaitingFirstExecution
                ? 'var(--text-muted)'
                : driftDetected
                ? 'var(--lvl-3)'
                : 'var(--lvl-0)',
        ...
        <div style={{ height: '6px', borderRadius: '3px', backgroundColor: 'rgba(255, 255, 255, 0.08)', overflow: 'hidden' }}>
          <div
            style={{
              height: '100%',
              width: awaitingFirstExecution ? '0%' : `${Math.min(100, Math.max(0, riskScore))}%`,
        ...
        <span style={{ display: 'block', marginTop: '8px', fontSize: '11px', color: 'var(--text-muted)' }}>
          {awaitingFirstExecution
            ? 'No actions evaluated yet — awaiting first task or sweep cycle'
