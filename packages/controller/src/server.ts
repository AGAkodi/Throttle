...
export function createControllerServer(options: ServerOptions): ThrottleServer {
  ...
  const server = http.createServer((req, res) => {
    ...
    // POST /api/actions/:id/approve: Operator approval for Level 4 pending actions
    if (req.method === 'POST' && url.pathname.startsWith('/api/actions/') && url.pathname.endsWith('/approve')) {
      const parts = url.pathname.split('/');
      const actionId = parts[3];
      const records = store.getActionRecords(undefined, 100);
      const target = records.find(r => r.id === actionId || r.action.id === actionId);
      if (!target) {
        res.writeHead(404);
        res.end(JSON.stringify({ error: `Action ${actionId} not found` }));
        return;
      }
      target.executionStatus = 'executed';
      target.executionTxHash = target.executionTxHash || `0xapproved_${Date.now().toString(16)}`;
      store.saveActionRecord(target);

      const agent = store.getAgent(target.agentId);
      if (agent) {
        agent.metrics.successfulActions += 1;
      ...
      res.writeHead(200);
      res.end(JSON.stringify({ success: true, record: target }));
      return;
    }

    // POST /api/actions/:id/reject: Operator rejection for Level 4 pending actions
    if (req.method === 'POST' && url.pathname.startsWith('/api/actions/') && url.pathname.endsWith('/reject')) {
      const parts = url.pathname.split('/');
      const actionId = parts[3];
      const records = store.getActionRecords(undefined, 100);
      const target = records.find(r => r.id === actionId || r.action.id === actionId);
      ...
      target.executionStatus = 'rejected';
      target.errorMessage = 'Manually rejected by operator via Throttle Console';
      store.saveActionRecord(target);

      const agent = store.getAgent(target.agentId);
      if (agent) {
        agent.metrics.failedActions += 1;
        store.saveAgent(agent);
      }
